...
Translator.prototype = {
	...
	translate: function(tree) {
		...
		switch(tree.getType()) {
			...
			case Type.ENVIRONMENT:
				this.translateEnvironment(tree)
				break
			case Type.PROGRAM:
	...
	translateEnvironment: function(tree) {
		fs.appendFileSync(this.outfile, '{')
		let table = tree.getLeft()
		let vars = table.getLeft()
		let vals = table.getRight()
		let flag = false
		if (vars != null) {
		...
		while (vars != null) {
			this.printTabs()
			fs.appendFileSync(this.outfile, vars.getLeft().getValue() + ': ')
			this.translate(vals.getLeft())
			vars = vars.getRight()
			vals = vals.getRight()
			if (vars != null)
				fs.appendFileSync(this.outfile, ',')
...
wjs.prototype = {
	...
	addVariables: function() {
		this.addGuaranteed()
		this.addNotGuaranteed()
		this.addParams()
		this.addCallback()
		this.addInsertCode()
	},

	addGuaranteed: function() {
		this.addWorkspaceId()
		this.addIamApikey()
		this.addVersion()
		this.addUrl()
	...
	addWorkspaceId: function() {
		fs.writeFileSync(this.outfile, '')
		this.addGenericVar('workspace_id')
	},

	addIamApikey: function() {
		this.addGenericVar('iam_apikey')
	},

	addVersion: function() {
		this.addGenericVar('version')
	},

	addUrl: function() {
		this.addGenericVar('url')
	},

	addWatson: function() {
		fs.appendFileSync(this.outfile, 'var watson = require(\'watson-developer-cloud\');')
	},

	addAssistant: function() {
		fs.appendFileSync(this.outfile, 'var assistant = new watson.AssistantV1({\n')
		fs.appendFileSync(this.outfile, '\tiam_apikey: iam_apikey,\n')
		fs.appendFileSync(this.outfile, '\tversion: version,\n')
		fs.appendFileSync(this.outfile, '\turl: url\n')
		fs.appendFileSync(this.outfile, '});\n')
	},

	addGenericVar: function(str) {
		let variable = new Lexeme(Type.ID, str, undefined, undefined, undefined)
		let result = Environment.prototype.lookup(this.global, variable)
		fs.appendFileSync(this.outfile, 'var ' + str + ' = \'' + result.getValue() + '\';\n')
	},

	isGuaranteed: function(str) {
		if (str == 'workspace_id' ||
			str == 'iam_apikey' ||
			str == 'version' ||
			str == 'url' ||
	...
	addNotGuaranteed: function() {
		let table = this.global.getLeft()
		let vars = table.getLeft()
		let vals = table.getRight()
		while (vars != null) {
			if (!this.isGuaranteed(vars.getLeft().getValue())) {
				fs.appendFileSync(this.outfile, 'var ' + vars.getLeft().getValue() + ' = ')
				let translator = new Translator(this.outfile)
				translator.translate(vals.getLeft())
				fs.appendFileSync(this.outfile, ';\n')
			}
			vars = vars.getRight()
			vals = vals.getRight()
	...
	addParams: function() {
		fs.appendFileSync(this.outfile, 'var params = {}\n')
		...
		if (Environment.prototype.exists(this.global, workspace_id))
			this.injectParam('params', 'workspace_id')
		if (Environment.prototype.exists(this.global, intents))
			this.injectParam('params', 'intents')
		if (Environment.prototype.exists(this.global, entities))
			this.injectParam('params', 'entities')
		if (Environment.prototype.exists(this.global, dialog_nodes))
			this.injectParam('params', 'dialog_nodes')
	},

	injectParam: function(obj, value) {
		fs.appendFileSync(this.outfile, '' + obj + '[\'' + value + '\'] = ' + value + ';\n')
	},

	addCallback: function() {
		fs.appendFileSync(this.outfile, 'function callback(err, response) {\n')
		fs.appendFileSync(this.outfile, '\tif (err)\n')
		fs.appendFileSync(this.outfile, '\t\tconsole.error(err);\n')
		fs.appendFileSync(this.outfile, '\telse\n')
		fs.appendFileSync(this.outfile, '\t\tconsole.log(JSON.stringify(response, null, 2));\n')
		fs.appendFileSync(this.outfile, '}\n')
	},

	addInsertCode: function() {
		fs.appendFileSync(this.outfile, 'assistant.updateWorkspace(params, callback);')
	}
